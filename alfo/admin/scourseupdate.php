...
if(is_uploaded_file($_FILES['image']['tmp_name'])) {
	...
	if(move_uploaded_file($sourcePath,$targetPath)) {
		...
		$result = $db_handle->runQuery("UPDATE `signal_course` SET  `category`='".$cat."',`course`='".$tit."',`price`='".$pri."',`details`='".$det."',`description`='".$des."' ,`image`='".$targetPath."' where id='".$id."'");
...
$result1 = $db_handle->runQuery("UPDATE `signal_course` SET  `category`='".$cat."',`course`='".$tit."',`price`='".$pri."',`details`='".$det."',`description`='".$des."' where id='".$id."'");
//echo "UPDATE `comment` SET  `name`='".$name."',`email`='".$email."',`subject`='".$subject."',`comment`='".$message."' where id='".$id."'";
//echo "UPDATE `contact` SET  `location`='".$name."',`email`='".$email."',`phone_number`='".$subject."' where id='".$id."'";
echo '<script>alert(" Updated Successfully ");
window.location.href="signalcourse.php"; </script>';
